...
$(document).ready(function () {
  ...
  // When DOM is ready, select the container element and call the jQCloud method, passing the array of words as the first argument.
  $("#notesOntologiesCloudChart").jQCloud(
    landscapeData["notes_ontologies_json_cloud"]
  );
});
      ...
      onMouseoverSegment: function (d) {
        if (chartTooltipLocked == false) {
          d3.select("#chartTooltip")
            ...
            .style("opacity", 1)
            .style("z-index", 1);
          d3.select("#chartTooltipValue").text(d.data.uri);
...
var naturalLanguagePie = createPie(
  "naturalLanguagePieChartDiv",
  landscapeData["natural_language_json_pie"],
  true
);

var licensePie = createPie(
  "licensePieChartDiv",
  landscapeData["licenseProperty_json_pie"],
  true
);

var prefLabelPie = createPie(
  "prefLabelPropertyPieChartDiv",
  landscapeData["prefLabelProperty_json_pie"],
  false
);

var synonymPie = createPie(
  "synonymPropertyPieChartDiv",
  landscapeData["synonymProperty_json_pie"],
  false
);

var definitionPie = createPie(
  "definitionPropertyPieChartDiv",
  landscapeData["definitionProperty_json_pie"],
  false
);

var authorPie = createPie(
  "authorPropertyPieChartDiv",
  landscapeData["authorProperty_json_pie"],
  false
);
...
var ontologyFormatsContext = document
  .getElementById("formatCanvas")
  .getContext("2d");
...
var ifOfTypeTooltipCallbacks = {
  title: function (tooltipItem, data) {
    return tooltipItem[0].yLabel.replace(
      "omv:",
      "http://omv.ontoware.org/2005/05/ontology#"
    );
  },
};
var isOfTypeContext = document
  .getElementById("isOfTypeCanvas")
  .getContext("2d");
...
var formalityTooltipCallbacks = {
  title: function (tooltipItem, data) {
    return tooltipItem[0].xLabel.replace(
      "nkost:",
      "http://w3id.org/nkos/nkostype#"
    );
  },
};
var formalityLevelContext = document
  .getElementById("formalityLevelCanvas")
  .getContext("2d");
...
// Vertical bar charts for ontologies formality levels
var dataCatalogContext = document
  .getElementById("dataCatalogCanvas")
  .getContext("2d");
...
var groupTooltipCallbacks = {
  ...
  beforeBody: function (tooltipItem, data) {
    return landscapeData["groupsInfoHash"][tooltipItem[0].xLabel][
      "description"
    ];
  },
};
var groupCountContext = document
  .getElementById("groupsCanvas")
  .getContext("2d");
...
var domainTooltipCallbacks = {
  ...
  beforeBody: function (tooltipItem, data) {
    return landscapeData["domainsInfoHash"][tooltipItem[0].xLabel][
      "description"
    ];
  },
};
var domainCountContext = document
...
var sizeSlicesContext = document
  .getElementById("sizeSlicesCanvas")
  .getContext("2d");
  ...
  for (var i = 0; i < ontologyRelationsArray.length; i++) {
    ...
    if (nodeIds[ontologyRelationsArray[i]["source"]] != null) {
      ...
      nodes.add([
        {
          id: sourceNodeNumber,
          label: ontologyRelationsArray[i]["source"],
          color: "#5cb85c",
        },
      ]);
    ...
    if (nodeIds[ontologyRelationsArray[i]["target"]] != null) {
      ...
      if (ontologyRelationsArray[i]["targetInPortal"] == true) {
        nodes.add([
          {
            id: targetNodeNumber,
            label: ontologyRelationsArray[i]["target"],
            color: "#5cb85c",
          },
        ]);
      } else {
    ...
    edges.add([
      {
        ...
        to: targetNodeNumber,
        label: ontologyRelationsArray[i]["relation"],
