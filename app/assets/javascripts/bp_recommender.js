...
rec.showOrHideAdvancedOptions = function () {
  ...
  var text = $("#advancedOptions").is(":visible")
...
rec.getHighlightedTerms = function (data, rowNumber) {
  ...
  for (var j = 0; j < data[rowNumber].coverageResult.annotations.length; j++) {
    ...
    var ontologyId =
      data[rowNumber].coverageResult.annotations[j].annotatedClass.links
        .ontology;
    ...
    var clsId =
      data[rowNumber].coverageResult.annotations[j].annotatedClass["@id"];
...
rec.getRecommendations = function () {
  ...
  if (!errors) {
    ...
    if ($("#radioItText").is(":checked")) params.input_type = 1; //text
    else params.input_type = 2; //keywords
    // Output type (ontologies or ontology sets)
    if ($("#radioOtSingle").is(":checked")) params.output_type = 1; //ontologies
      ...
      success: function (data) {
        ...
        if (data) {
          if (data.length > 0) {
            ...
            var table = $(
              '<table id="recommendations" class="zebra" width="100%"></table>'
            );
            var header = $(
            ...
            for (var i = 0; i < data.length; i++) {
              ...
              for (
                var j = 0;
                j < data[i].coverageResult.annotations.length;
                j++
              ) {
                terms +=
                  '<a target="_blank" href=' +
                  data[i].coverageResult.annotations[j].annotatedClass.links
                    .ui +
                  ">" +
