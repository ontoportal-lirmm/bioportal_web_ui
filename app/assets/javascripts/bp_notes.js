    ...
    jQuery("div.facebox_note")
      ...
      .css(
        ...
        jQuery(window).height() -
          (jQuery("#facebox").offset().top - jQuery(window).scrollTop()) * 2 +
          "px"
      );
...
function bindReplyCancelClick() {
  jQuery(".reply .cancel, .create_note_form .cancel").live(
    "click",
    function () {
      removeReplyBox(this);
    }
...
function bindProposalChange() {
  jQuery(".create_note_form .proposal_type").live("change", function () {
    ...
    proposalFields(
      selector.val(),
      selector.parent().children(".proposal_container")
    );
  });
}
...
var displayError = function (button) {
  jQuery(button)
    .parent()
    .children(".reply_status")
    .html("Error, please try again");
};

function addCommentBox(id, type, button) {
  var formContainer = jQuery(button)
    .parents(".notes_list_container")
    .children(".create_note_form");
  ...
  var commentWrapper = jQuery("<div>")
    .addClass("reply_box")
    .append(commentFields);
...
function addProposalBox(id, type, button) {
  ...
  for (var proposalType in NOTES_PROPOSAL_TYPES) {
    select.append(
      jQuery("<option>")
        .attr("value", proposalType)
        .html(NOTES_PROPOSAL_TYPES[[proposalType]])
    );
  }
  ...
  proposalForm.append(
    jQuery("<div>")
      .addClass("proposal_buttons")
      .append(commentButtons(id, type))
  );
...
function addReplyBox(button) {
  ...
  jQuery(button)
    .closest("div.reply")
    .children("div.reply_meta")
    .append(jQuery("<div>").addClass("reply_box").html(formHTML));
}
...
function proposalMap(button) {
  ...
  formContainer.find(".proposal_container input").each(function () {
    ...
    var val =
      jQuery.inArray(id, lists) >= 0 ? input.val().split(",") : input.val();
    map[id] = val;
  });
  return map;
}
...
function subjectForNote(button) {
  var subject = jQuery(button)
    .closest(".reply_box")
    .children(".comment_subject")
    .val();
  var reasonForChange = jQuery("input#reasonForChange");
  if (
    typeof subject === "undefined" ||
    (subject.length === 0 && reasonForChange.length > 0)
  ) {
    subject =
      NOTES_PROPOSAL_TYPES[$(".proposal_type").val()] +
      ": " +
      reasonForChange.val();
  }
  return subject;
...
function generateNoteLink(id, note) {
  return jQuery("<a>")
    ...
    .attr(
      "href",
      "/ontologies/" +
        jQuery(document).data().bp.ont_viewer.ontology_id +
        "/notes/" +
...
function subscribeToNotes(button) {
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      ...
      var newButtonText = txt.match("Unsubscribe")
        ? txt.replace("Unsubscribe", "Subscribe")
        : txt.replace("Subscribe", "Unsubscribe");
      jQuery("a.subscribe_to_notes").html(newButtonText);
    },
    error: function (data) {
      jQuery(".notes_subscribe_spinner").hide();
      jQuery(".notes_sub_error").html(
        "Problem subscribing to emails, please try again"
      );
    },
  });
}
