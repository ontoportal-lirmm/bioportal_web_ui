...
export default class extends Controller {
  ...
  connect() {
    this.simpleTreeCollection = useSimpleTree(
      this.element,
  ...
  #onClickTooManyChildrenInit() {
    jQuery(".too_many_children_override").live("click", (event) => {
      ...
      jQuery.ajax({
        ...
        error: function () {
          this.html(
            "<div style='background: #eeeeee; padding: 5px; width: 80%;'>Problem getting children. <a href='" +
              jQuery(this).attr("href") +
              "' class='too_many_children_override'>Try again</a></div>"
          );
        },
      });
    });
