...
export default class extends Controller {
  ...
  #updateColorsTags(collectionElem) {
    ...
    let activeCollections = this.#getMatchedCollections(
      collectionElem,
      collections,
      this.selected
    );
    ...
    this.#addColorsTags(
      collectionElem,
      this.#getCollectionColors(activeCollections)
    );
  }
  ...
  #collectionsChoices() {
    const options = document.getElementById(
      this.collectionsColorSelectTargetValue
    );
    const out = {};
    if (options) {
  ...
  #getMatchedCollections(elem, collections, selected) {
    collections = [
      ...new Set(collections.concat(this.#getElemActiveCollections(elem))),
    ];
    return selected.filter((c) => collections.includes(c));
  }

  #getCollectionColors(collectionsIds) {
    return Object.entries(this.allCollections)
      .filter(([key]) => collectionsIds.includes(key))
      .map((x) => x[1]);
  }
