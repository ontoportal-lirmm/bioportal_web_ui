...
export default class extends Controller {
  ...
  connect() {
    ...
    jQuery(document).ready(() => {
      useBioportalAutoComplete(
        this.element,
        BP_INTERNAL_SEARCH_SERVER + "/search/json_search/",
        {
    ...
    if (
      document.getElementById(inputName + "_bioportal_preferred_name") == null
    )
  ...
  formatItem(row) {
    ...
    if (ontology_id !== "all") {
      ...
      if (BP_include_definitions) {
        result +=
          "<div class='result_definition'>" +
          this.#truncateText(
            decodeURIComponent(result_def.replace(/\+/g, " ")),
            75
          ) +
          "</div>";
      }
      ...
      result +=
        "<div class='result_class' style='width: " +
        class_name_width +
        ";'>" +
        result_class.replace(
          regex,
          "<b><span class='result_class_highlight'>$1</span></b>"
        ) +
        "</div>";

      result +=
        "<div class='result_type' style='overflow: hidden;'>" +
        result_type +
        "</div>";
    } else {
      // Results
      ...
      result +=
        ...
        result_type +
        "</div><div class='result_ontology' style='overflow: hidden;'>" +
  ...
  onSelect(li) {
    ...
    jQuery(`input[name="${input_name}_bioportal_ontology_id"]`).val(
      li.extra[2]
    );
    jQuery(`input[name="${input_name}_bioportal_full_id"]`).val(li.extra[3]);
    jQuery(`input[name="${input_name}_bioportal_preferred_name"]`).val(
      li.extra[4]
    );
    this.#emitOnSelect();
  }
