...
export default class extends Controller {
  ...
  #setColors(selected) {
    ...
    const allChosenSelected = Array.from(
      document.querySelectorAll(
        `#${snakeCase(this.element.id + "_chosen")} .search-choice`
      )
    );
    selected.forEach((s) => {
      let color = s.dataset.color;
      if (color) {
        const chosenSelected = allChosenSelected
          .filter((x) => x.firstElementChild.textContent === s.text)
          .pop();
