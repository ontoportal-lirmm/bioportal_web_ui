...
class FairScoreChartContainer {
  constructor(fairChartsContainerId, charts) {
    ...
    this.requestHrefBase =
      this.requestLink != null ? this.requestLink.attr("href") : "";

    this.fairSpinner = jQuery(
      "<div id='fair-spinner-container' class='w-100 text-center'> <div class='spinner-grow'></div> </div>"
    );
    this.fairMsgErr = jQuery(
      "<div id='fair-msg-container' class='w-100 text-center'> We could not collect the data from the fairness service</div>"
    );
  ...
  #fillScoreSpans(data) {
    if (this.fairAverageScoreSpan) {
      this.fairAverageScoreSpan.html(
        printScore(data.score, data.normalizedScore)
      );
    }

    if (data.resourceCount > 1) {
      this.#showScoreLabel(
        this.fairMinScoreSpan,
        data.minScore,
        data.maxCredits
      );
      this.#showScoreLabel(
        this.fairMaxScoreSpan,
        data.maxScore,
        data.maxCredits
      );
      this.#showScoreLabel(
        this.fairMedianScoreSpan,
        data.medianScore,
        data.maxCredits
      );
    } else {
  ...
  #hideScoreLabel(elem) {
    if (elem) elem.parent().parent().hide();
  }
}
...
class FairScoreChart {
  ...
  setFairScoreData(data) {
    if (this.fairScoreChartCanvas) {
      Object.entries(data[this.dataField]).forEach(([key, value]) =>
        this.fairScoreChartCanvas.data(key, value)
      );
      this.fairScoreChartCanvas.data("resourceCount", data["resourceCount"]);
      if (this.chart === null) this.chart = this.initChart();
      else {
...
class FairScorePrincipleBar extends FairScoreChart {
        ...
        tooltips: {
          callbacks: {
            label: function (tooltipItem, data) {
              let score = jQuery(this._chart.canvas).data("scores")[
                tooltipItem.index
              ];
              let maxScore = jQuery(this._chart.canvas).data("maxCredits")[
                tooltipItem.index
              ];
              let portalMaxScore = jQuery(this._chart.canvas).data(
                "portalMaxCredits"
              )[tooltipItem.index];
              ...
              return printScore(
                [score, notObtained, na][tooltipItem.datasetIndex],
                normalizedScore
              );
            },
          },
  ...
  getFairScoreDataSet() {
    ...
    const { scores, notObtained, na } = getObtainedNotObtainedNA(
      this.fairScoreChartCanvas.data("scores"),
      portalMaxCredits,
      maxCredits
    );
    return [
      ...
      if (tooltipModel.body) {
        ...
        titleLines.forEach(function (title, index) {
          innerHtml +=
            ...
            title +
            " : " +
            descriptions[tooltipModel.dataPoints[0].index] +
            "</th></tr>";
        });
    ...
    return function (tooltipModel) {
      ...
      if (tooltipModel.body) {
        ...
        bodyLines.forEach(function (body, i) {
          ...
          style += "; width: 100%";
          innerHtml +=
            '<span class="btn card-subtitle m-2 text-muted" style="' +
            style +
            '">' +
            body +
            "</span>";
        ...
        innerHtml +=
          '</div> <ul class="list-group list-group-flush" style="font-size: medium">';

        for (const [key, value] of Object.entries(
          questions[tooltipModel.dataPoints[0].index]
        )) {
          let count = value.state
            ? value.state.success + value.state.average
            : value.score === value.maxCredits
            ? 1
            : 0;
          innerHtml += `<li class="list-group-item">
                        <strong>${printScore(
                          count,
                          round((count / resourceCount) * 100)
                        )}</strong>
        ...
        tooltips: {
          ...
          callbacks: {
            label: function (tooltipItem, data) {
              ...
              const score =
                data.datasets[tooltipItem.datasetIndex].data[tooltipItem.index];
              const normalizedScore = Object.values(
                getObtainedNotObtainedNA(scores, portalMax, max, false)
              )[tooltipItem.datasetIndex][tooltipItem.index];
...
jQuery("#fairness_assessment").ready(() => {
  let fairCriteriaBars = new FairScoreCriteriaBar(
    "ont-fair-scores-criteria-bars-canvas"
  );
  let fairContainer = new FairScoreChartContainer(
    "fair-score-charts-container",
    [fairCriteriaBars]
  );
...
jQuery("#fair-home").ready(function (e) {
  ...
  let fairScoreRadar = new FairScoreCriteriaRadar(
    "ont-fair-criteria-scores-canvas"
  );
  let fairContainer = new FairScoreChartContainer(
    "fair-score-charts-container",
    [fairScoreRadar, fairScoreBar]
  );
  let ontologies = jQuery("#ontology_ontologyId");
